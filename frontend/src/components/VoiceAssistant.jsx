...
const supportedLanguages = [
  ...
  const speak = useCallback((text) => {
    ...
    // Prevent rapid-fire speaks (debounce)
    const now = Date.now();
    const timeSinceLastSpeak = now - lastSpeakTimeRef.current;
    
    // If less than 100ms since last speak, it's likely a duplicate - skip it
    if (timeSinceLastSpeak < 100) {
      console.log('Skipping duplicate speak call');
      return;
    }
    ...
    setTimeout(() => {
      ...
      const langCode = currentLanguage.split('-')[0]; // e.g., 'te' from 'te-IN'
      
      // Priority 1: Look for exact match (e.g., te-IN)
      selectedVoice = voices.find(v => v.lang === currentLanguage);
      
      // Priority 2: Look for language code match (e.g., te)
      if (!selectedVoice) {
        selectedVoice = voices.find(v => v.lang.startsWith(langCode));
      }
      
      // Priority 3: Fallback to English if target language not available
      if (!selectedVoice) {
        console.warn(`⚠️ No voice found for ${currentLanguage}.`);
        console.log('🔄 Falling back to English voice for audio feedback');
        
        // Try to find any English voice
        selectedVoice = voices.find(v => v.lang.startsWith('en-')) || voices.find(v => v.lang.startsWith('en')) || voices[0];
        
        if (selectedVoice) {
          utterance.voice = selectedVoice;
          utterance.lang = selectedVoice.lang; // Use English lang code for better pronunciation
          console.log(`📢 Using fallback voice: ${selectedVoice.name} (${selectedVoice.lang})`);
          
          // Add note to history that we're using fallback
          setResponse(text);
          addToHistory('assistant', text + ` 🌐`);
        }
      } else {
        utterance.voice = selectedVoice;
        console.log(`✅ Found voice for ${currentLanguage}:`, selectedVoice.name);
      ...
      utterance.onerror = (event) => {
        ...
        // Don't log "interrupted" errors as they're expected when changing commands quickly
    ...
    }, 150); // Increased delay to ensure clean cancellation
  }, [currentLanguage]);
  ...
  const changeLanguage = (langCode) => {
    ...
    // Wait for state update and then speak in the new language
    setTimeout(() => {
      speak(`${languageResponses[langCode]?.changeLanguage || 'Changing language'} - ${langName}`);
    }, 200);
  };
